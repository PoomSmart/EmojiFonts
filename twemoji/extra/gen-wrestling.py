...
def get_x_coords(element):
    ...
    if tag == 'path':
        ...
        # Per SVG spec, an initial lowercase m is treated as absolute.
        stripped = d.lstrip()
        if stripped and stripped[0] == 'm':
            match = re.match(r'm\s*([-+]?\d*\.\d+|\d+)', stripped)
            if match:
                coords.append(float(match.group(1)))
...
def get_split_x(root):
    # Use viewBox center when available (e.g. "0 0 36 36").
...
def apply_silhouette(element):
    for child in element:
        if 'fill' in child.attrib:
            child.set('fill', silhouette_color)
...
# 1f9d1 + 1faef -> 1f93c
# 1f468 + 1faef -> 1f93c-200d-2642-fe0f
# 1f469 + 1faef -> 1f93c-200d-2640-fe0f
gender_map = {
    '1f9d1': '1f93c',
    '1f468': '1f93c-200d-2642-fe0f',
    '1f469': '1f93c-200d-2640-fe0f',
}
